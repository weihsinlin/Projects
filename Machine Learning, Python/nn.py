...
class Graph(object):
    ...
    def get_output(self, node):
        ...
        # print "output!!!!"
        # print node.forward(self.get_inputs(node))
    ...
    def add(self, node):
        ...
        out = self.get_output(node)
        self.dict[node] = [out, np.zeros_like(out)]
    ...
    def backprop(self):
        ...
        "*** YOUR CODE HERE ***"
        back_list = self.nodes[::-1]
        self.dict[loss_node][1] = 1. # np.identity(loss_node.forward(loss_node.get_parents).shape[1])
        for node in back_list: 
            # print "111111111111"
            # print self.dict[node][1]
            g_list = node.backward(self.get_inputs(node), self.dict[node][1])

            parent = node.get_parents()
            if parent != []: 
                for i in range(len(g_list)): 
                    # print "111111222222"
                    # print g_list[i]

                    self.dict[parent[i]][1] += g_list[i]
    ...
    def step(self, step_size):
        ...
        for node in self.nodes:
            if isinstance(node, Variable):
                node.data -= self.dict[node][1] * step_size
...
class MatrixVectorAdd(FunctionNode):
    ...
    def backward(inputs, gradient):
        ...
        return [gradient, np.dot(ones.T, gradient)[0]]
...
class ReLU(FunctionNode):
    ...
    @staticmethod
    def backward(inputs, gradient):
        index = np.where(inputs[0] < 0)
        new_gradient = np.copy(gradient)
        new_gradient[index] = 0
        return [new_gradient]
...
class SquareLoss(FunctionNode):
    ...
    @staticmethod
    def forward(inputs):
        return (0.5 * ((inputs[0] - inputs[1]) ** 2)).mean()

    @staticmethod
    def backward(inputs, gradient):
        gradient1 = np.array((inputs[0] - inputs[1]) * gradient / len(inputs[0]))
        gradient2 = np.array((inputs[1] - inputs[0]) * gradient / len(inputs[1]))
